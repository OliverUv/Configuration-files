...
if [ $# -ne $EXARGS ] ; then
    ...
    Videos downloaded are converted to audio-only files.
EOF
    ...
    while read p; do
        ...
        youtube-dl --download-archive 'archive-log' --ignore-errors -f best -o '%(playlist)s/%(playlist_index)s-%(title)s-%(id)s.%(ext)s' --extract-audio --audio-format 'vorbis' "$p" 2>&1 | tee -a $TMPFILE
    done < "$1"
    echo ""
    echo ""
    echo "Summary:"
